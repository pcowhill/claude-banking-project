...
| `feedback/FEEDBACK_v0.7.0_2026-06-27_0107.md` | v0.7.0 | 2026-06-27 | **Clean approval** of v0.7.0 — Money movement ("Everything looks great") + go-ahead to start **v0.8.0 — Cards, fraud, disputes**. One small **optional UI request** (left to Claude's judgement), **accepted** as task **`R-03`**: when an operator **Reverses** a money movement, the queue item badge still reads "Approved" only — add a secondary **"Reversed"** tag wherever the request is listed (queue cards, dashboard, detail), derived from the payload/ledger; the request stays terminal-approved (no state-machine change). Folded into v0.8.0 since disputes also produce reversals. No re-scope; recurring/scheduled payments stay deferred to v0.9.0. |
| `feedback/FEEDBACK_v0.8.0_2026-06-27_0219.md` | v0.8.0 | 2026-06-27 | **Clean "continue"-style approval** of v0.8.0 — Cards, fraud, disputes ("All looks good so far. Keep up the good work!"). No bugs, no change requests, no re-scope. Interpreted as approval to proceed with **v0.9.0 — Simulation clock + recurring/scheduled payments** (+ statement cycles) per ROADMAP / NEXT_SESSION. The carried `M-09` (recurring/scheduled payments, deferred from v0.7.0 because it needs the clock) is pulled into v0.9.0 and realized by the new `SP-xx` tasks. |
| `feedback/FEEDBACK_v0.9.0_2026-06-29_0105.md` | v0.9.0 | 2026-06-29 | **Approval to start v1.0.0**, plus a **re-scope** of v1.0.0 (feature + hardening + polish). Four accepted items: (1) **stale marketing placeholders** — Cards shipped in v0.8.0 yet the homepage tile + `/cards` still say "coming"; Loans & CDs not built but mis-dated "v0.9.0" → build the feature then make copy truthful (`PH-xx`); (2) **time-travel date bug** — a clock-fired bill pay posts/approves on the **wall-clock** date, not the simulated date → audit all money/ops/scheduler timestamps onto the sim clock (`DT-xx`); (3) **loans / CDs / interest accrual pulled into v1.0.0** ("If so, do them") → built on the disciplined ledger + clock + scheduler (`LN-xx`); (4) go-ahead to v1.0.0. Hardening (`HD-xx`: SEC-1 CSRF, L-2/L-3, dev-tooling audit), test expansion (`QA-xx`), and the capstone retrospective (`RT-xx`) remain. |
