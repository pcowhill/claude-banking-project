...
to continue the experiment after reviewing **v0.6.1**. Replace
...
Current state: patch release v0.6.1 (Operations console fixes) is COMPLETE and
tagged. It addressed the two bugs from the v0.6.0 review and started NO new
feature work:
  - B-03: on a narrow window the operations console's left sidebar disappeared
    with no alternative; added an accessible top-bar menu (☰) so Dashboard /
    Request queues / Simulated messaging stay reachable at any width.
  - B-04: the operations console showed "Not authenticated" in Request queues and
    blocked approvals when the operator session was invalid/expired/stale; the
    console now reconciles a rejected session (HTTP 401) and returns the operator
    to the sign-in screen with a clear notice, then recovers on re-login. The
    backend + onboarding-approval path were verified correct end-to-end; the
    defect was client-side session handling only.
The milestone BEFORE this (v0.6.0 — Onboarding and account opening) remains the
last feature milestone: a real open-account flow feeding the v0.5.0 ops queue; an
operator approval that PROVISIONS a user + account + initial funding (bank-
originated, posted `deposit`, audited; balances DERIVED); joint-account
invitations; admin-created demo users. The next planned milestone is
...
5. Then implement ONLY the next approved milestone (v0.7.0 unless my feedback
   ...
   customer dashboard, the v0.5.0 operations console, or v0.6.0 onboarding (incl.
   the v0.6.1 ops-console fixes — narrow-width nav and session recovery).
...
docs/process/EXPERIMENT_LOG.md (Sessions 1–7). None of this affects normal
machines or CI.
...
## My human review feedback for v0.6.1
