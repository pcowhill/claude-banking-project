...
## Milestone v0.9.0 — Simulation clock + recurring/scheduled payments (+ statement cycles)  ✅ Done (tag `v0.9.0` created locally; human pushes on merge)
...
## Milestone v1.0.0 — Polish, hardening, loans/CDs/interest, final retrospective  ▶️ In Progress

> **Approved to start AND re-scoped by the human** (see
> `feedback/FEEDBACK_v0.9.0_2026-06-29_0105.md`). v1.0.0 was planned as a pure
> hardening-and-finish pass; the human's v0.9.0 review **pulls loans / CDs / interest
> accrual into v1.0.0** ("Now that the clock is in place, can these be pursued within
> the v1.0.0 as well? If so, do them.") and reports two polish/correctness items:
> **stale marketing placeholders** (Cards shipped in v0.8.0 yet the homepage tile +
> `/cards` still say "coming v0.8.0"; Loans & CDs mis-dated "v0.9.0") and a
> **time-travel date bug** (a clock-fired bill pay posts/approves on the **wall-clock**
> date, not the simulated date). So v1.0.0 is a combined **feature + hardening +
> polish** capstone — the experiment's final milestone, ending in the capstone
> retrospective + an annotated `v1.0.0` tag.
>
> **Risky shared areas serialized + reviewed first** (the constitution's list — schema,
> auth/**CSRF**, routing, real-time, the **ledger**, the **clock/scheduler**, CI,
> architecture): the **simulated-date fix** (`V-01`, touches the ledger/clock dating
> path), the **lending shared contract** (`L-01`), the **`LendingProduct` migration**
> (`L-02`, one additive migration — a new table + one nullable `Account` column; no
> existing table altered), the **lending services + clock-driven accrual** (`L-03`,
> ledger-only), the **lending routes** (`L-04`, contract LOCKED here), and **CSRF**
> (`H-01`, touches the cookie/guard auth seam + every frontend API client). Contracts
> are locked before the two frontends (`L-06`/`L-07`) are parallelized.
>
> **MONEY DISCIPLINE (unchanged, enforced + tested):** loans / CDs / interest accrual
> move money ONLY by appending `LedgerEntry` rows — never a stored/edited balance.
> **Interest accrual** posts a **bank-originated `interest`** credit (savings/CD) or a
> bank-originated `interest` debit (loan interest owed), dated at the **simulated**
> accrual date. **Opening a CD** and **a loan disbursement / payment** post paired
> `transfer` legs that **net to zero** (cash moves between the customer's checking and
> the CD/loan account; the loan account simply carries a negative — owed — balance).
> Balances stay DERIVED. **`V-01`** makes the **simulation clock the single
> authoritative "now" for every money/business timestamp** (transfers, movements, ops
> approvals/reversals, disputes/fraud, onboarding/admin funding, cards, accrual, events)
> — **superseding ADR-0002 decision #2**; auth/session/security/operational timestamps
> (session expiry, login history, lockout, the heartbeat/status `serverTime`) stay
> wall-clock by design (they track real elapsed time). Recorded in **ADR-0003**.
> Simulation disclaimer stays visible; no regression to v0.2.0 auth / v0.3.0 site /
> v0.4.0 dashboard / v0.5.0 ops / v0.6.0–v0.6.2 onboarding+fixes / v0.7.0 money movement
> / v0.8.0 cards/fraud/disputes / v0.9.0 clock/scheduler/statements.

| ID | Title | Role | Acceptance criteria | Deps | Status | Result |
| --- | --- | --- | --- | --- | --- | --- |
| V-01 | Simulated-date correctness (everything uses the sim clock) | Backend/API (risky, serial) | The reported time-travel bug + its whole class: thread `simulationNow(prisma)` into **every** money/ops/business route — `routes/money.ts` (transfer/movement), `routes/ops.ts` (action **incl. the bill-pay approve that showed the wall-clock date**, reverse, simulate-event, admin create+funding), `routes/onboarding.ts` (submit/invite/accept/decline), `routes/risk.ts` (dispute/fraud-respond), `routes/cards.ts` (issue/freeze/unfreeze/report/travel). Keep **auth/session/lockout/heartbeat/status `serverTime`** on wall-clock (operational/security — documented). Add a deterministic `id` tiebreaker to `toTransactionDTOs` so same-simulated-instant entries order stably (resolves the ADR-0002 #2 "monotonic ordering" concern). Regression tests assert a fired→approved bill pay carries the **simulated** date. **Supersedes ADR-0002 #2; recorded in ADR-0003** | v0.9.0 | In&nbsp;Progress | — |
| V-02 | Marketing placeholder accuracy | Frontend Customer | Update the homepage product tiles (`MarketingHome`) so **Cards** ("Arrives v0.8.0") and **Loans & CDs** ("Arrives v0.9.0") read as **live** features (e.g. "Explore now"/"In your portal"); rewrite `/cards` (`Cards.tsx`) and `/borrow` (`Borrow.tsx`) to present shipped features (cards now; loans/CDs once `L-06` lands) with **no false version-dated "coming soon"** copy; keep the simulation disclaimer + clearly-simulated framing. No portal/auth regression | V-01 | In&nbsp;Progress | — |
| L-01 | Lending shared contract + math | Backend/Shared (risky, serial) | New `packages/shared/src/lending.ts`: `LENDING_KINDS` (`cd`/`loan`), `LENDING_STATUSES` (`active`/`matured`/`paid_off`/`closed`), default savings APY + bounds, DTOs (`LendingProductDTO`, open-CD / open-loan / loan-payment / CD-withdraw request+response), PURE math (`monthlyAccrual`, `amortizedPayment`, `projectMaturityValue`, period iteration that is calendar-safe like `addInterval`) + validators (`validateOpenCd`/`validateOpenLoan`/`validateLoanPayment`). Dependency-free; unit-tested; barrel export; **contract locked before UI** | v0.9.0 | Pending | — |
| L-02 | `LendingProduct` schema migration | Backend/API (risky, serial) | Additive Prisma migration `lending`: `LendingProduct` (1:1 `Account`; `kind`, `status`, `principalMinor`, `apyBps`, `termMonths`, `openedAt`, `maturesAt`, `lastAccruedAt`, nullable `paymentMinor` for loans) + a nullable `Account.interestAccruedThrough DateTime?` (savings live accrual). `cd`/`loan` already exist in `ACCOUNT_TYPES`; `interest`/`fee` already in `LEDGER_ORIGINS`. **No existing table altered** (only the additive column + new table). Generated through the mirrored schema engine; `db:reset` green | L-01 | Pending | — |
| L-03 | Lending services + clock-driven accrual | Backend/API (risky, serial) | `apps/backend/src/lending/lending.ts`: `openCd` (debit funding account + credit CD account — net zero — + `LendingProduct`), `openLoan` (debit loan account −principal + credit checking +principal — net zero — + amortized `paymentMinor`), `makeLoanPayment` (checking→loan, net zero; insufficient → typed error), `withdrawMaturedCd` (CD→checking at/after maturity). `apps/backend/src/lending/accrual.ts`: `runInterestAccrual(upTo)` — for each active savings (default APY) + CD (product APY) post a bank-originated `interest` **credit**; for each active loan post an `interest` **debit** on outstanding; monthly cadence, **bounded catch-up**, dated at the simulated period end, idempotent via `lastAccruedAt`/`interestAccruedThrough`. **Wire into `POST /api/ops/clock/advance`** right after `runDueSchedules`. Ledger-only; balances derived; audited | L-02 | Pending | — |
| L-04 | Lending routes (+ lock contract) | Backend/API (risky, serial) | `routes/lending.ts` (customer, `requireAuth`, access-scoped): `POST /api/lending/cds` (open), `POST /api/lending/loans` (open), `GET /api/lending` (the user's lending products w/ derived balances + product terms), `POST /api/lending/loans/:id/pay`, `POST /api/lending/cds/:id/withdraw`. Ops visibility via existing account reads / a `GET /api/ops/lending` if needed. Register in `routes/index.ts`; use the shared validators; **API contract LOCKED here** before frontends | L-03 | Pending | — |
| L-05 | Seed: demo CD + loan + savings APY | Backend/API | Extend the seed PLAN: give Avery a **CD** (funded from savings, mid-term, due to mature after a modest advance) and a **personal loan** (disbursed to checking, amortized), and set savings to accrue at the default APY; a `lending` seed-plan section keyed to declared accounts + `assertSeedLendingIntegrity`; existing money/ops/card/schedule invariants still pass; `db:reset` green | L-02,L-03 | Pending | — |
| L-06 | Customer lending UI | Frontend Customer | `lib/lending.ts` client (mirrors `lib/money.ts`). A portal **Loans & CDs** experience: open a CD / open a loan (shared validators + MoveMoney form primitives), list lending products with derived balances + terms (APY, term, maturity, next payment), **make a loan payment**, **withdraw a matured CD**; surface **savings interest** accrual on the savings detail/dashboard. Dashboard quick link. Loading/error/empty states; disclaimer prominent; finalize `V-02` Loans & CDs copy to match the shipped feature | L-04 | Pending | — |
| L-07 | Operations lending visibility | Frontend Operations | The ops console can see customers' lending products (read-only, ops/admin) and the interest entries an advance produced (the `/clock` fired summary already lists scheduled fires; add an accrual summary if surfaced by `L-03`). No new money-moving ops action; live sync + disclaimer not regressed | L-04 | Pending | — |
| H-01 | CSRF protection (SEC-1) | Backend/API + Shared + both Frontends (risky, serial) | Real CSRF defense for state-changing POSTs (transfers, movements, schedules, lending, clock advance, ops actions, onboarding, disputes/fraud, admin): a **double-submit token** — backend sets a non-httpOnly `mer_csrf` cookie + a `requireCsrf` preHandler that checks a matching `x-meridian-csrf` header on mutating requests; both `lib/api.ts` clients read the cookie and send the header. Login/logout bootstrap a token. Keep `SameSite=Lax` + CORS. Reproduce-then-guard tests (missing/mismatched token → 403; valid → ok); session isolation + all flows not regressed | V-01 | Pending | — |
| H-02 | Ledger/scheduler hardening (L-2/L-3) | Backend/API + Security | Disposition the carried **L-2** (scheduler run-count/last-run bookkeeping is a separate write) and **L-3 / v0.7.0 F-2** (funds-check TOCTOU, now also reachable from the scheduler + accrual): either tighten within SQLite's constraints (e.g. fold the funds check into the write path where practical) or document the accepted residual risk for a single-user simulation in `QUALITY_REPORT.md` + ADR-0003. No lost/created dollar; balances derived | L-03 | Pending | — |
| H-03 | Dev-tooling npm-audit advisories | Testing/QA + Security | Audit + remediate or document the final disposition of the dev-only vite/vitest/esbuild advisories (tracked since v0.1.0). Runtime `npm audit --omit=dev` must stay **0**. If major upgrades risk destabilizing the toolchain right before the 1.0 tag, record the formal accepted disposition in `QUALITY_REPORT.md` rather than forcing a risky bump | v0.9.0 | Pending | — |
| Q-01 | First frontend component unit tests | Testing/QA | Introduce the long-deferred frontend component unit tests (jsdom + Testing Library): cover a few logic-bearing components (e.g. `TransactionList` grouping/filter, a lending or money form's validation wiring). Wire into the test runner so `npm run test` includes them | L-06 | Pending | — |
| Q-02 | Backend + e2e coverage for v1.0.0 | Testing/QA | Backend integration: lending open/pay/withdraw (net-zero + money invariant), **interest accrual** on advance (bank-originated entries; balances derived; idempotent; catch-up), the `V-01` simulated-date regression, CSRF (`H-01`) accept/reject, RBAC on every new route. Playwright: open a CD/loan, advance the clock and see interest accrue + a bill-pay approved on the **simulated** date, the corrected marketing pages. `npm run verify` + `npm run test:e2e` green | V-01,L-04,H-01 | Pending | — |
| Q-03 | Security review | Security Reviewer | Read-only security/sim-safety audit of the whole v1.0.0 surface: CSRF correctness, RBAC/IDOR on lending routes, the owner-scoped accrual/lending privilege boundary, money discipline (ledger-only, net-zero, bank-originated interest, derived balances, no balance edits), the simulated-date change introduces no back-dating exploit, no secrets, simulation safety. Findings dispositioned before the gate | Q-02 | Pending | — |
| R-01 | Capstone retrospective + experiment report | Process Scribe | The experiment's final retrospective: what the milestone-gated, repo-as-source-of-truth, controlled-multi-agent process produced across v0.1.0→v1.0.0; what worked, what would change; the final experiment report | Q-03 | Pending | — |
